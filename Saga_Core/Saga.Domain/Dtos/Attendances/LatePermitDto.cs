...
public class LatePermitDto
{
    ...
    public LatePermit ConvertToEntity()
    {
        return new LatePermit
        {
            Key = this.Key ?? Guid.NewGuid(),
            EmployeeKey = this.EmployeeKey ?? Guid.Empty,
            DateSubmission = this.DateSubmission ?? DateTime.Now,
            TimeIn = this.TimeIn ?? TimeOnly.FromDateTime(DateTime.Now),
...
public class LateDetailReportDto : GeneralAttendanceReportDto
{
